  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (typeof WebSocket !== 'undefined') {
        await performWebSocketQuery(userMessage.content)
      } else {
        // Fallback to HTTP
        await performHttpQuery(userMessage.content)
      }
    } catch (error) {
                ...
                <div key={message.id} className={`flex gap-4 ${message.role === "user" ? "justify-end" : ""}`}>
                  {message.role === "assistant" || message.role === "sources" || message.role === "overview" ? (
                    <div className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 ${
                      message.role === "sources" ? "bg-blue-100" : 
                      message.role === "overview" ? "bg-purple-100" : 
                      "bg-primary/10"
                    }`}>
                  ...
                  <Card
                    className={`max-w-[80%] transition-all duration-300 ${
                      ...
                      message.role === "sources" ? "border-blue-200 dark:border-blue-800 bg-blue-50/50 dark:bg-blue-950/50" :
                      message.role === "overview" ? "border-purple-200 dark:border-purple-800 bg-purple-50/50 dark:bg-purple-950/50" :
                      ""
                    }`}
                  >
