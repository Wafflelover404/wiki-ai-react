...
import { metricsApi, filesApi } from "@/lib/api"
...
export default function UserDashboard() {
    ...
    async function fetchData() {
      ...
      try {
        ...
        if (metricsRes.status === "success" && metricsRes.response) {
          ...
          setMetrics(metricsRes.response as MetricsSummary)
        }
  ...
  const successRate = metrics?.success_rate ?? (metrics ? Math.round((metrics.successful_queries / (metrics.total_queries || 1)) * 100) : 0)
  ...
  return (
    <>
      ...
      <main className="flex-1 p-6 space-y-6">
        ...
        {/* Stats Grid - Personal Stats Only */}
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          <Card>
