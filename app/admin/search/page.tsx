  ...
  const handleAiAgentCommand = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.status === "success") {
        ...
        if (
          result.response &&
          (result.response.snippets || result.response.results)
        ) {
            ...
            result.response.results.forEach(
              (resultItem: any, index: number) => {
                searchResults.push({
                  ...
                  type: "document",
                  title:
                    resultItem.metadata?.original_filename ||
                    resultItem.title ||
                    resultItem.source ||
                    `AI Result ${index + 1}`,
                  content:
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    try {
        ...
        onMessage: (message) => {
          ...
          switch (message.type) {
            case "status":
              ...
              if (
                message.message &&
                message.message !== "Processing query..."
              ) {
                setMessages((prev) => [
                  ...prev,
                  {
            ...
            case "immediate":
              ...
              if (
                message.data &&
                (message.data.snippets || message.data.results)
              ) {
                if (message.data.snippets) {
                  message.data.snippets.forEach(
            ...
            case "overview":
              const finalContent = aiAgentMode
      ...
      if (result && (result as any).status === "success") {
        toast.success(
          aiAgentMode ? "AI Agent search completed!" : "Search completed!",
        );
      }
    } catch (error) {
      setActiveAnalysis(null);
                ...
                <div className="space-y-4">
                  ...
                  <form onSubmit={handleSearch} className="space-y-4">
                      ...
                      <Input
                        ...
                        placeholder={
                          aiAgentMode
                            ...
                            : "Search documents, products, or ask questions..."
                        }
                    ...
                    <Button
                      type="submit"
                      disabled={isLoading}
                      className={`w-full ${aiAgentMode ? "bg-purple-600 hover:bg-purple-700" : ""}`}
                    >
                      {isLoading ? (
                        <>
              ...
              <CardContent>
                <ScrollArea className="h-[400px] w-full" ref={scrollRef}>
                  {messages.map((message, index) => (
                    <div
                      key={message.id}
                      className={`mb-4 p-4 rounded-lg ${message.role === "user" ? "bg-blue-50 dark:bg-blue-950" : message.role === "assistant" ? "bg-green-50 dark:bg-green-950" : "bg-muted"}`}
                    >
                  ...
                  {activeAnalysis && activeAnalysis.type === "loading" && (
                    <div
                      key="active-loading"
                      className="mb-4 p-4 rounded-lg bg-green-50 dark:bg-green-950 border border-green-100 dark:border-green-900 animate-pulse"
                    >
                      <div className="flex items-start gap-3">
                        <div className="flex-shrink-0">
                  ...
                  {activeAnalysis && activeAnalysis.type === "streaming" && (
                    <div
                      ...
                      className="mb-4 p-4 rounded-lg bg-green-50 dark:bg-green-950 border border-green-200 dark:border-green-800 shadow-sm transition-all duration-200"
                    >
                      <div className="flex items-start gap-3">
                        <div className="flex-shrink-0">
